...
pub const SemanticAnalyzer = struct {
    ...
    fn inferType(self: *SemanticAnalyzer, node_id: ast.NodeId) !?ast.Type {
        ...
        switch (node.data) {
            .literal => |literal| {
                return switch (literal) {
                    .integer => |int_lit| ast.Type.initPrimitive(
                        int_lit.type_hint orelse .{ .i32 = {} },
                        node.source_loc
                    ),
                    .float => |float_lit| ast.Type.initPrimitive(
                        float_lit.type_hint orelse .{ .f64 = {} },
                        node.source_loc
                    ),
                    .string => ast.Type.initPrimitive(.{ .string = {} }, node.source_loc),
            ...
            .match_expr => |match_expr| {
                ...
                _ = try self.inferType(match_expr.expression);
    ...
    fn checkMemberAccess(
                    ...
                    if (base_node) |base| {
                        if (base.data == .member_expr) {
                            ...
                            if (std.mem.eql(u8, member.field, "List")) {
                                const std_obj_node = self.arena.getNode(member.object);
                                if (std_obj_node) |std_obj| {
                                    if (std_obj.data == .identifier) {
                                        const ident = std_obj.data.identifier;
                                        if (std.mem.eql(u8, ident.name, "std")) {
                                            // This is std.List(T).method()
                                            if (std.mem.eql(u8, field_name, "init")) {
                                                // Return a function type for init
                                                return ast.Type.initPrimitive(.{ .type = {} }, source_loc);
                                            } else if (std.mem.eql(u8, field_name, "from")) {
                                                // Return a function type for from
                                                return ast.Type.initPrimitive(.{ .type = {} }, source_loc);
                                            } else if (std.mem.eql(u8, field_name, "initCapacity")) {
                                                // Return a function type for initCapacity
                                                return ast.Type.initPrimitive(.{ .type = {} }, source_loc);
                                            }
                                        }
                                    }
        ...
        const object_type = try self.inferType(object_id);
        if (object_type == null) return null;
        
        const obj_type = object_type.?;
    ...
    pub fn typesCompatible(self: *SemanticAnalyzer, a: ast.Type, b: ast.Type) bool {
        _ = self;
        ...
        if (a.data == .custom_struct and b.data == .custom_struct) {
            return std.mem.eql(u8, a.data.custom_struct.name, b.data.custom_struct.name);
        }
        
        // Regular struct compatibility
        if (a.data == .@"struct" and b.data == .@"struct") {
            return std.mem.eql(u8, a.data.@"struct".name, b.data.@"struct".name);
        }
        
        // Mixed struct compatibility (custom vs regular)
        if ((a.data == .custom_struct and b.data == .@"struct") or 
            (a.data == .@"struct" and b.data == .custom_struct)) {
            const name_a = if (a.data == .custom_struct) a.data.custom_struct.name else a.data.@"struct".name;
            const name_b = if (b.data == .custom_struct) b.data.custom_struct.name else b.data.@"struct".name;
            return std.mem.eql(u8, name_a, name_b);
        }
        
        // TODO: Implement more sophisticated type compatibility
        return false;
    }
    
    fn isBoolean(self: *SemanticAnalyzer, typ: ast.Type) bool {
        _ = self;
        return typ.data == .primitive and typ.data.primitive == .bool;
    }
    
    fn isNumeric(self: *SemanticAnalyzer, typ: ast.Type) bool {
        _ = self;
    ...
    fn checkStructInitialization(
        ...
        if (struct_init.type_name) |type_name| {
            ...
            if (self.struct_definitions.get(type_name)) |struct_def| {
                const struct_type = struct_def.struct_type;
                ...
                for (struct_init.fields.items) |field_init| {
                    try provided_fields.put(field_init.name, true);
                    ...
                    for (struct_type.fields) |field_def| {
                        if (std.mem.eql(u8, field_def.name, field_init.name)) {
                            ...
                            const value_type = try self.inferType(field_init.value);
                            if (field_def.type_annotation) |field_type_id| {
                                const field_type = try self.inferType(field_type_id);
                                if (value_type != null and field_type != null) {
                                    if (!self.typesCompatible(field_type.?, value_type.?)) {
                                        const msg = try std.fmt.allocPrint(
                                            self.allocator,
                                            "Field '{s}' type mismatch in struct initialization",
                                            .{field_init.name}
                                        );
                                        defer self.allocator.free(msg);
                                        try self.reportError(.type_mismatch, msg, source_loc);
                                    }
                                }
                            }
                            break;
                ...
                for (struct_type.fields) |field_def| {
                    ...
                    if (!is_method and !provided_fields.contains(field_def.name) and field_def.default_value == null) {
                        ...
                        defer self.allocator.free(msg);
                        try self.reportError(.invalid_declaration, msg, source_loc);
                    }
                }
                
                // Return the struct type
                return ast.Type.initCustomStruct(type_name, struct_type.fields, struct_type.is_comptime, source_loc);
            } else {
                const msg = try std.fmt.allocPrint(
                    self.allocator,
            ...
            // Anonymous struct initialization - infer type from fields
            var field_types = std.ArrayList(ast.Field).init(self.allocator);
            defer field_types.deinit();
            
            for (struct_init.fields.items) |field_init| {
                const value_type = try self.inferType(field_init.value);
                if (value_type != null) {
                    // Create a field with inferred type
                    const field = ast.Field{
                        .name = field_init.name,
                        .type_annotation = null, // Would need to create a type node
                        .default_value = null,
                        .source_loc = source_loc,
                    };
                    try field_types.append(field);
                }
            }
            
            return ast.Type.initCustomStruct("", try field_types.toOwnedSlice(), true, source_loc);
        }
    }
    ...
    fn checkArrayInitialization(
        ...
        if (array_init.elements.items.len == 0) {
            // Empty array - return unknown array type
            return ast.Type{
                .data = .{ .array = .{ .element_type = undefined, .size = 0 } },
                .source_loc = source_loc,
            };
        }
        
        // Infer element type from first element
        const first_element_type = try self.inferType(array_init.elements.items[0]);
        if (first_element_type == null) return null;
        
        // Check that all elements have compatible types
        for (array_init.elements.items[1..]) |element_id| {
            const element_type = try self.inferType(element_id);
            if (element_type != null) {
                if (!self.typesCompatible(first_element_type.?, element_type.?)) {
                    try self.reportError(
                        .type_mismatch,
        ...
        // Create array type (we'll need to allocate the element type)
        const element_type_ptr = try self.createTrackedType(first_element_type.?);
        
        return ast.Type{
            .data = .{ .array = .{ .element_type = element_type_ptr, .size = array_init.elements.items.len } },
            .source_loc = source_loc,
        };
    }
    ...
    pub fn analyzeNode(self: *SemanticAnalyzer, node_id: ast.NodeId) anyerror!?ast.Type {
        const node = self.arena.getNode(node_id) orelse return null;
        
        switch (node.data) {
            .var_decl => |var_decl| {
                return self.analyzeVariableDeclaration(var_decl, node.source_loc);
            },
            ...
            .type_decl => |type_decl| {
                return self.analyzeTypeDeclaration(type_decl, node.source_loc);
            },
            
            .import_decl => |import_decl| {
                return self.analyzeImportDeclaration(import_decl, node.source_loc);
            },
            
            .return_stmt => |return_stmt| {
                return self.analyzeReturnStatement(return_stmt, node.source_loc);
            },
            ...
            .block => |block| {
                ...
                var last_type: ?ast.Type = null;
                for (block.statements.items) |stmt_id| {
                    last_type = try self.analyzeNode(stmt_id);
                }
            ...
            .for_expr => |for_expr| {
                return self.analyzeForExpression(for_expr, node.source_loc);
            },
            ...
            .range_expr => |range_expr| {
                ...
                // For expressions, infer the type
                const inferred_type = try self.inferType(node_id);
                
                // Update the node with type information
                if (inferred_type) |typ| {
                    const mutable_node = self.arena.getNode(node_id).?;
                    mutable_node.type_info = typ;
                }
    ...
    fn analyzeVariableDeclaration(
        ...
        if (declared_type != null and inferred_type != null) {
            if (!self.typesCompatible(declared_type.?, inferred_type.?)) {
                try self.reportError(
                    .type_mismatch,
                    "Variable initializer type doesn't match declared type",
                    source_loc
                );
            }
        }
    ...
    fn analyzeStructDeclaration(
        ...
        const comptime_value = ComptimeValue{ 
            .struct_type = .{ 
                .name = struct_decl.name, 
                .fields = struct_decl.fields.items,
                .is_comptime = struct_decl.is_comptime,
    ...
    fn analyzeTypeDeclaration(
        ...
        if (comptime_value == null or !comptime_value.?.isType()) {
            try self.reportError(
                .invalid_declaration,
                "Type declaration must evaluate to a type",
                source_loc
            );
            return null;
        }
        ...
        if (resolved_type == null) {
            try self.reportError(
                .invalid_declaration,
                "Could not resolve type expression",
                source_loc
            );
            return null;
        }
                ...
                if (!std.mem.eql(u8, capture.name, "_")) { // Don't declare ignored captures
                    ...
                    self.current_scope.declare(capture.name, symbol) catch |err| {
                        if (err == error.DuplicateSymbol) {
                            const msg = try std.fmt.allocPrint(
                                self.allocator,
                                "Capture variable '{s}' already declared",
                                .{capture.name}
                            );
                            defer self.allocator.free(msg);
    ...
    fn handleAlignOfBuiltin(
        ...
        if (args.len != 1) {
            try self.reportError(
                .invalid_function_call,
                "@alignOf expects exactly one argument",
                source_loc
            );
            return null;
        }
    ...
    fn handleOffsetOfBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@offsetOf expects exactly two arguments",
                source_loc
            );
            return null;
        }
    ...
    fn handleFieldTypeBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@FieldType expects exactly two arguments",
                source_loc
            );
            return null;
        }
    ...
    fn handleVectorBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@Vector expects exactly two arguments",
                source_loc
            );
            return null;
        }
    ...
    fn handleSplatBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@splat expects exactly two arguments",
                source_loc
            );
            return null;
        }
    ...
    fn handleSafeArithmeticBuiltin(
        ...
        if (args.len != 2) {
            const msg = try std.fmt.allocPrint(
                self.allocator,
                "@{s}_s expects exactly two arguments",
                .{operation}
            );
            defer self.allocator.free(msg);
    ...
    fn handleCastUpBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@castUp expects exactly two arguments",
                source_loc
            );
            return null;
        }
    ...
    fn handleCastDownBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@castDown expects exactly two arguments",
                source_loc
            );
            return null;
        }
    ...
    fn handleTruncateBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@truncate expects exactly two arguments",
                source_loc
            );
            return null;
        }
    ...
    fn handleIntCastBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@intCast expects exactly two arguments",
                source_loc
            );
            return null;
        }
    ...
    fn handleFloatCastBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@floatCast expects exactly two arguments",
                source_loc
            );
            return null;
        }
    ...
    fn handleCompileErrorBuiltin(
        ...
        if (args.len != 1) {
            try self.reportError(
                .invalid_function_call,
                "@compileError expects exactly one argument",
                source_loc
            );
            return null;
        }
        ...
        const msg = try std.fmt.allocPrint(
            self.allocator,
            "Compile-time error triggered by @compileError",
            .{}
        );
        defer self.allocator.free(msg);
    ...
    fn handleMemcpyBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@memcpy expects exactly two arguments",
                source_loc
            );
            return null;
        }
    ...
    fn handleMemsetBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@memset expects exactly two arguments",
                source_loc
            );
            return null;
        }
    ...
    fn handleBitCountBuiltin(
        ...
        if (args.len != 2) {
            const msg = try std.fmt.allocPrint(
                self.allocator,
                "@{s} expects exactly two arguments",
                .{operation}
            );
            defer self.allocator.free(msg);
            try self.reportError(.invalid_function_call, msg, source_loc);
    ...
    fn handleArrayListBuiltin(
        ...
        if (args.len != 1) {
            try self.reportError(
                .invalid_function_call,
                "@ArrayList expects exactly one type argument",
                source_loc
            );
            return null;
        }
    ...
    fn handleArrayListInitBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@arrayListInit expects exactly two arguments (type, allocator)",
                source_loc
            );
            return null;
        }
    ...
    fn handleArrayListAppendBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@arrayListAppend expects exactly two arguments (list, item)",
                source_loc
            );
            return null;
        }
    ...
    fn handleArrayListGetBuiltin(
        ...
        if (args.len != 2) {
            try self.reportError(
                .invalid_function_call,
                "@arrayListGet expects exactly two arguments (list, index)",
                source_loc
            );
            return null;
        }
    ...
    fn handleArrayListLenBuiltin(
        ...
        if (args.len != 1) {
            try self.reportError(
                .invalid_function_call,
                "@arrayListLen expects exactly one argument (list)",
                source_loc
            );
            return null;
        }
    ...
    fn handleArrayListClearBuiltin(
        ...
        if (args.len != 1) {
            try self.reportError(
                .invalid_function_call,
                "@arrayListClear expects exactly one argument (list)",
                source_loc
            );
            return null;
        }
    ...
    fn checkUnusedSymbols(self: *SemanticAnalyzer) void {
        ...
        while (iterator.next()) |entry| {
            ...
            if (!symbol.is_used and symbol.symbol_type == .variable) {
                ...
                self.reportWarning(
                    .undefined_variable, // Reusing error code for warning
                    msg,
                    symbol.source_loc
                ) catch {};
            }
                ...
                if (obj_node.data == .identifier) {
                    ...
                    if (std.mem.eql(u8, ident.name, "std")) {
                        ...
                        if (generic_type.type_params.items.len != 1) {
                            try self.reportError(
                                .type_mismatch,
                                "std.List expects exactly one type parameter",
                                source_loc
                            );
                            return null;
                        }
        ...
        if (base_node.data == .identifier) {
            ...
            if (std.mem.eql(u8, ident.name, "List")) {
                ...
                if (generic_type.type_params.items.len != 1) {
                    try self.reportError(
                        .type_mismatch,
                        "List expects exactly one type parameter",
                        source_loc
                    );
                    return null;
                }

...
pub const Parser = struct {
    ...
    fn parseCall(self: *Parser) !ast.NodeId {
        ...
        while (true) {
            if (self.match(&[_]std.meta.Tag(Token){.LeftParen})) {
                ...
                expr = try self.arena.createNode(ast.AstNode.init(.{ .index_expr = .{ .object = expr, .index = index } }, source_loc));
                ...
                if (name_token == .Identifier) {
                    ...
                    expr = try self.arena.createNode(ast.AstNode.init(.{ .member_expr = .{ .object = expr, .field = field_name } }, source_loc));
                }
            } else if (self.match(&[_]std.meta.Tag(Token){.Caret})) {
                // Handle postfix dereference: expr^
                const source_loc = self.getCurrentSourceLoc();
                expr = try ast.createUnaryExpr(self.arena, source_loc, .deref, expr);
            } else if (self.match(&[_]std.meta.Tag(Token){.LeftBrace})) {
                // Handle struct initialization: MyStruct{ .field1 = value1, .field2 = value2 }
                expr = try self.finishStructInitialization(expr, false);
            } else {
    ...
    fn finishCall(self: *Parser, callee: ast.NodeId) !ast.NodeId {
        ...
        if (try self.isTypeConstructor(callee)) {
            ...
            return self.arena.createNode(ast.AstNode.init(.{ .generic_type_expr = .{ .base_type = callee, .type_params = args } }, source_loc));
        } else {
            // This is a regular function call
            return self.arena.createNode(ast.AstNode.init(.{ .call_expr = .{ .callee = callee, .args = args } }, source_loc));
        }
    }

    fn finishStructInitialization(self: *Parser, struct_type: ast.NodeId, use_gc: bool) !ast.NodeId {
        const source_loc = self.getCurrentSourceLoc();
        ...
        if (self.check(.RightBrace)) {
            ...
            return self.arena.createNode(ast.AstNode.init(.{ .struct_init = .{
                .type_name = type_name,
                .fields = fields,
                .use_gc = use_gc,
            } }, source_loc));
    ...
    fn parsePrimary(self: *Parser) !ast.NodeId {
                ...
                if (std.mem.eql(u8, builtin_name, "compile")) {
                    ...
                    if (member_token == .Identifier) {
                        ...
                        if (std.mem.eql(u8, member_name, "target")) {
                            ...
                            const member_expr = try self.arena.createNode(ast.AstNode.init(.{ .member_expr = .{ .object = compile_id, .field = member_name } }, source_loc));
        ...
        if (self.match(&[_]std.meta.Tag(Token){.Dot})) {
            if (self.check(.LeftBrace)) {
                ...
                return self.arena.createNode(ast.AstNode.init(.{ .call_expr = .{ .callee = struct_name, .args = args } }, source_loc));
                ...
                return ast.createLiteralExpr(self.arena, source_loc, ast.Literal{ .enum_member = .{ .name = member_name } });
            } else {
        ...
        if (self.match(&[_]std.meta.Tag(Token){.Dollar})) {
            ...
            if (self.match(&[_]std.meta.Tag(Token){.LeftBracket})) {
                ...
                return self.arena.createNode(ast.AstNode.init(.{ .array_init = .{ .elements = elements, .use_gc = true } }, source_loc));
            }

            // Parse struct allocation: $MyStruct{...}
            if (self.check(.Identifier)) {
                const type_name_token = self.advance();
                const type_name = if (type_name_token == .Identifier) type_name_token.Identifier.value else "unknown";
                ...
                if (self.match(&[_]std.meta.Tag(Token){.LeftBrace})) {
                    // Create type identifier node
                    const type_node = try ast.createIdentifier(self.arena, source_loc, type_name);

                    // Parse struct initialization with heap allocation
                    return self.finishStructInitialization(type_node, true);
                } else {
                    try self.reportError(.invalid_expression, "Expected '{' after struct type in $ allocation", source_loc);
                    return ast.createIdentifier(self.arena, source_loc, "error");
                }
            }

            // For now, handle other cases as errors
            try self.reportError(.invalid_expression, "$ allocation supports arrays [$[...]] and structs [$Type{...}]", source_loc);
        ...
        if (self.match(&[_]std.meta.Tag(Token){.LeftBracket})) {
            ...
            return self.arena.createNode(ast.AstNode.init(.{ .array_init = .{ .elements = elements, .use_gc = false } }, source_loc));
        }
                ...
                if (self.check(.ColonEquals)) {
                    ...
                    if (self.check(.Fn)) {
                        ...
                        const func_result = self.parseHowlFunctionDeclaration() catch |err| {
                            ...
                            return self.arena.createNode(ast.AstNode.init(.{ .error_node = .{
    ...
    fn parseHowlFunctionDeclaration(self: *Parser) !ast.NodeId {
        ...
        return self.arena.createNode(ast.AstNode.init(.{ .function_decl = .{
            .name = name,
            .params = params,
    ...
    fn parseForStatement(self: *Parser) !ast.NodeId {
        ...
        if (self.check(.DotDotLessThan) or self.check(.DotDotEquals)) {
            ...
            iterable = try self.arena.createNode(ast.AstNode.init(.{ .range_expr = .{
                .start = null,
                .end = end_expr,
    ...
    fn parseReturnStatement(self: *Parser) !ast.NodeId {
        ...
        return self.arena.createNode(ast.AstNode.init(.{ .return_stmt = .{ .value = value } }, source_loc));
    }
    ...
    fn parseEnumDeclaration(self: *Parser, name: []const u8, source_loc: ast.SourceLoc) !ast.NodeId {
        ...
        return self.arena.createNode(ast.AstNode.init(.{ .enum_decl = .{
            .name = name,
            .members = members,
        } }, source_loc));
    }
    ...
    fn parseIfExpression(self: *Parser) !ast.NodeId {
        ...
        return self.arena.createNode(ast.AstNode.init(.{ .if_expr = .{
            .condition = condition,
    ...
    fn parseMatchExpression(self: *Parser) !ast.NodeId {
        ...
        if (expr_node) |node| {
            if (node.data == .compile_target_expr) {
                ...
                return self.arena.createNode(ast.AstNode.init(.{ .match_compile_expr = .{ .target_expr = expression, .arms = compile_arms } }, source_loc));
            }
        }

        // Transfer ownership of arms ArrayList to AST node
        return self.arena.createNode(ast.AstNode.init(.{ .match_expr = .{ .expression = expression, .arms = arms } }, source_loc));
    }
    ...
    fn parseType(self: *Parser) !ast.NodeId {
        ...
        // Handle pointer types (e.g., ^i32, ^MyStruct)
        if (self.match(&[_]std.meta.Tag(Token){.Caret})) {
            const inner_type = try self.parseType();
            return ast.createPointerTypeExpr(self.arena, source_loc, inner_type);
        }

...
pub const SemanticErrorHelpers = struct {
    ...
    pub fn undefinedVariable(
        ...
        const message = try std.fmt.allocPrint(self.allocator, "undefined variable '{s}'", .{variable_name});
        ...
        for (available_variables) |var_name| {
            if (self.calculateLevenshteinDistance(variable_name, var_name) <= 2) {
                try suggestions.append(try std.fmt.allocPrint(self.allocator, "did you mean '{s}'?", .{var_name}));
            }
        }
    ...
    pub fn typeMismatch(
        ...
        const message = try std.fmt.allocPrint(self.allocator, "type mismatch in {s}: expected '{s}', found '{s}'", .{ context, expected_type, actual_type });
        defer self.allocator.free(message);
        ...
        if (std.mem.eql(u8, expected_type, "i32") and std.mem.eql(u8, actual_type, "str")) {
            ...
            try suggestions.append(try std.fmt.allocPrint(self.allocator, "cast the value to '{s}' if the conversion is intentional", .{expected_type}));
        }
    ...
    pub fn duplicateDeclaration(
        ...
        const message = try std.fmt.allocPrint(self.allocator, "duplicate declaration of '{s}'", .{identifier_name});
        defer self.allocator.free(message);
        ...
        const related_message = try std.fmt.allocPrint(self.allocator, "'{s}' was previously declared here", .{identifier_name});
    ...
    pub fn invalidMemberAccess(
        ...
        const message = try std.fmt.allocPrint(self.allocator, "type '{s}' has no member '{s}'", .{ base_type, member_name });
        defer self.allocator.free(message);
        ...
        for (available_members) |member| {
            if (self.calculateLevenshteinDistance(member_name, member) <= 2) {
                try suggestions.append(try std.fmt.allocPrint(self.allocator, "did you mean '{s}'?", .{member}));
            }
        }
    ...
    pub fn invalidFunctionCall(
        ...
        const message = switch (error_kind) {
            .not_a_function => try std.fmt.allocPrint(self.allocator, "'{s}' is not a function", .{function_name}),
            .wrong_argument_count => |counts| try std.fmt.allocPrint(self.allocator, "function '{s}' expects {d} argument{s}, but {d} {s} provided", .{ function_name, counts.expected, if (counts.expected == 1) "" else "s", counts.actual, if (counts.actual == 1) "was" else "were" }),
            .argument_type_mismatch => |mismatch| try std.fmt.allocPrint(self.allocator, "argument {d} of function '{s}': expected '{s}', found '{s}'", .{ mismatch.argument_index + 1, function_name, mismatch.expected_type, mismatch.actual_type }),
        };
        defer self.allocator.free(message);
    ...
    pub fn binaryOperationTypeError(
        ...
        const message = try std.fmt.allocPrint(self.allocator, "binary operation '{s}' is not supported between '{s}' and '{s}'", .{ operator, left_type, right_type });
        defer self.allocator.free(message);
    ...
    pub fn returnTypeMismatch(
        ...
        const message = try std.fmt.allocPrint(self.allocator, "function '{s}' expects return type '{s}', but '{s}' was returned", .{ function_name, expected_type, actual_type });
        defer self.allocator.free(message);

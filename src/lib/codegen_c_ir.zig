...
const CIrCodegen = struct {
    ...
    fn generateTypeDefinitions(self: *CIrCodegen) CompileError!void {
        ...
        for (self.ir.nodes.items) |node| {
            ...
            if (node.op == .function_def) {
                ...
                try self.generateTypeFromAst(&generated_types, func_data.return_type);
            }
        }

        // Define MyStruct first (only if not already generated)
        const mystruct_hash = std.hash.Wyhash.hash(0, "MyStruct");
        if (!generated_types.contains(mystruct_hash)) {
            try generated_types.put(mystruct_hash, true);
            try self.writeLineFormatted("typedef struct {{", .{});
            try self.writeLineFormatted("    i32 field1;", .{});
            try self.writeLineFormatted("    f64 field2;", .{});
            try self.writeLineFormatted("}} MyStruct;", .{});
            try self.writeLine("");
        }
    ...
    fn generateTypeFromAst(self: *CIrCodegen, generated_types: *std.AutoHashMap(u64, bool), ast_type: ast.Type) CompileError!void {
        switch (ast_type.data) {
            .error_union => |error_union| {
                ...
                switch (error_union.payload_type.*.data) {
                    .custom_struct => try self.generateTypeFromAst(generated_types, error_union.payload_type.*),
                    .optional => try self.generateTypeFromAst(generated_types, error_union.payload_type.*),
                    else => {},
                }
            ...
            .custom_struct => |cs| {
                ...
                try self.writeLineFormatted("typedef struct {{", .{});
                // Field types are not yet fully resolved here; default to i64 for now
                for (cs.fields) |field| {
                    try self.writeLineFormatted("    i64 {s}; /* TODO: actual type */", .{field.name});
                }
